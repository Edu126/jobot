...
Single-value scoring: the LLM returns ONE overall 0-100 score per job,
plus a one-sentence reasoning and the top matched / gap terms. The backend
maps the score to the verdict bands for the ring colour. This is the
...
Every result has: score (0-100, straight from the model), verdict (enum,
derived from the score), one-sentence reasoning, and top matched/gaps.
Cached in SQLite per (resume_id, job_id, lang), gated on
(prompt_version, scoring_version) — a version bump logically invalidates
old rows without deleting them.
...
class ScoreResult:
    job_id: str
    score: int                              # 0-100, straight from the model
    verdict: str                            # derived from score bands
    reasoning: str                          # LLM one-sentence overall reasoning
    matched: list[str]                      # top matches (<=5), for the card + detail
    gaps: list[str]                         # top gaps (<=5), for the card + detail
    model: str
...
def score_single_no_cache(
    ...
    *,
    ...
    comes from the original resume's profile, since tailoring doesn't change
    who the candidate is. Pass `persona` directly instead when the caller
    already resolved it (e.g. the tailor route also calls `rewrite_resume`
    for the same resume_id in the same request — resolve once, pass to both).
    ...
    try:
        results = _score_batch(
            resume_snippet, [job], client, lang=lang, persona=persona,
        )
    except QuotaExhaustedError:
        return None
...
def save_scores(resume_id: int, scores: list[dict], lang: str) -> int:
    ...
    `sections`/`hard_requirements` are no longer produced; `db.save_scores`
    defaults those columns to empty, so the score dicts omit them."""
