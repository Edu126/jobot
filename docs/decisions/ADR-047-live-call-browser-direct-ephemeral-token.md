...
Date: 2026-09-16
Status: Accepted (design) — implementation gated on G1 (see REQ-041 / research
memo: free-tier confirmation).
...
- The Live session is a **new call pattern** (streaming, not `generate_json`),
  which ADR-008 rule 7 requires an ADR for. This is that ADR. It gets its own
  row in `llm-surface.md`, marked as streaming.
...
- **Long-lived API key in the client.** Never. It is the user's quota and our
  key (GOV-001/GOV-008).
